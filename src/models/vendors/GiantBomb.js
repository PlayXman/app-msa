...
class GiantBomb {
	...
	static searchGame( gameTitle, callback ) {
		GiantBombDb.getApiKey().then( ( apiKey ) => {
			const urlParams = new URLSearchParams( {
				api_key: apiKey,
				format: 'jsonp',
				query: Url.encodeText(gameTitle),
				field_list: 'name,id,image,original_release_date,expected_release_day,expected_release_month,expected_release_quarter,expected_release_year',
				resources: 'game'
			} );

			jsonpRequest( `${Config.vendors.giantBombCom.apiUrl}search/?${urlParams.toString()}` ).then( ( data ) => {
				if ( data.error === 'OK' ) {
	...
	static _fetchAllGames( urlParamsObj, itemCount, items, callback ) {
		...
		if ( urlParamsObj.offset <= itemCount ) {
			...
			jsonpRequest( `${Config.vendors.giantBombCom.apiUrl}games/?${urlParams.toString()}` ).then( ( data ) => {
				if ( data.error === 'OK' ) {
