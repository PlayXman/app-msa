...
class MediaPageContent extends Component {
	...
	handleFirstLoad = ( snap ) => {
		...
		snap.forEach( ( snapItem ) => {
			const item = this.mediaModel.createItem().setDefaults().fillObj( snapItem.val() ).setId( snapItem.key );
		...
		return (
			<PageContent>
				<Grid container spacing={0}>
					<Grid item xs>
						<Grid container justify="space-between" alignItems="center" className={classes.heading}>
							<Grid item xs>
